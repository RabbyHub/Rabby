...
export type BalanceViewType = {
  getBalanceText: () => string;
};
const BalanceView = React.forwardRef<BalanceViewType, Props>(
  ({ currentAccount }, ref) => {
    const { t } = useTranslation();

    useImperativeHandle(ref, () => ({
      getBalanceText: () => {
        return formatUsdValue(currentBalance || 0)?.toString() || '';
      },
    }));
    ...
    const shouldShowLoading =
      shouldShowBalanceLoading || shouldShowCurveLoading;
