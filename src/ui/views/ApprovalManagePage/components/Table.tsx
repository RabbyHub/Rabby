...
export function VirtualTable<RecordType extends object>({
  ...
  // reset hover row index when mouse leave the table
  useLayoutEffect(() => {
    if (!markHoverRow) return;

    const gridInst = gridRef.current;
    if (!gridInst) return;
    // @ts-expect-error `_outerRef` is the real HTML element generated by react-window, with className assgined from property on `VGrid``
    const htmlElement = gridInst._outerRef as HTMLElement;
    if (!htmlElement) return;

    const listener = () => {
      setHoveredRowIndex(null);
    };
    htmlElement.addEventListener('mouseleave', listener);

    return () => {
      htmlElement.removeEventListener('mouseleave', listener);
    };
  }, [markHoverRow]);
