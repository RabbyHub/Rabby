...
import { LucideCopy, LucideEllipsisVertical } from 'lucide-react';
...
const AddressItem = memo(
  ({
    ...
    const [isPrivateKeyModalOpen, setIsPrivateKeyModalOpen] = useState<boolean>(
      false
    );
    ...
    const lessThan5Minutes = (
      time1: Date | number,
      time2: Date | number
    ): boolean => {
      // Convert inputs to timestamps if they are Date objects
      const timestamp1 = typeof time1 === 'number' ? time1 : time1.getTime();
      const timestamp2 = typeof time2 === 'number' ? time2 : time2.getTime();
    ...
    const isPrivateKeyImported =
      BRAND_ALIAN_TYPE_TEXT[brandName] === 'Private Key';
    ...
    return (
      <>
        <ContextMenu.Root>
          <ContextMenu.Trigger>
            ...
            >
              ...
              <Avatar
                ...
                fallback=""
                color="gray"
                src={eachAccount.avatar}
                alt={`${eachAccount.name} avatar`}
              />
            )}*/}
                <Flex justify={'between'} align={'center'} width={'100%'}>
                  <Flex direction={'column'} align={'start'}>
                    <Text
                      ...
                      weight={'medium'}
                    >
                      {KEYRING_TYPE_TEXT[type] || type}
                    </Text>
                    <Text as="div" size="2" weight="bold">
                      {alias}{' '}
                      {/*{lessThan5Minutes(
                          new Date(),
                          new Date(eachAccount.datetime!).getTime(),
                        ) &&
                        accounts.length > 2 &&
                        index > 1 && (
                          <Badge
                    ...
                    <Text as="div" color="gray" weight={'bold'}>
                      {/*{eachAccount.ensName || shortenAddress(eachAccount.address)}*/}
                      {ellipsisAddress(address)}
                      {/*{BRAND_ALIAN_TYPE_TEXT[brandName]} -{' '}*/}
                      {/*{KEYRING_TYPE_TEXT[type] || type}*/}
                    </Text>
                  </Flex>
                  <Flex justify={'center'} align={'center'} gap={'4'}>
                    <Skeleton loading={isUpdatingBalance}>
                      <Text as="div" color="gray" weight={'bold'}>
          ...
          <ContextMenu.Content highContrast hideWhenDetached={false}>
            <ContextMenu.Item>
              <CopyTextComponent
                textToCopy={address}
                feedbackMessage={'Address copied'}
              >
            ...
            >
              {/* <EditNameModal currentAccount={eachAccount} /> */}
              <label htmlFor="id-edit-account-name">Edit name</label>
            </ContextMenu.Item>
            <ContextMenu.Separator />
            {/* <ContextMenu.Item>Show Account details</ContextMenu.Item> */}
            <ContextMenu.Item
              onClick={() => setIsPrivateKeyModalOpen(!isPrivateKeyModalOpen)}
            >
              <label htmlFor="id-show-private-key">Show Private Key</label>
            </ContextMenu.Item>
        ...
        <div
          className={clsx(
            className,
            'rabby-address-item-container relative hidden'
          )}
        >
                        ...
                        <>
                          ...
                          {whitelistEnable && isInWhiteList && (
                            <Tooltip
                              ...
                              placement="top"
                              title={t(
                                'page.manageAddress.whitelisted-address'
                              )}
                            >
                              <ThemeIcon
                                src={RcIconWhitelist}
                ...
                {enableSwitch && !isCurrentAccount && (
                  <div className="rabby-address-item-extra flex justify-center items-center pr-[12px]">
                    <div className="opacity-0 group-hover:opacity-100 w-[20px] h-[20px] rounded-full bg-blue-light flex items-center justify-center">
                      <img
                        src={IconCheck}
                        className="w-[54%] icon icon-check"
                      />
                    </div>
                  </div>
                )}
