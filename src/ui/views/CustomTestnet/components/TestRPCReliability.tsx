// ui/views/CustomTestnet/components/TestRPCReliability.tsx
...
const TestRPCReliability = ({ chainData }) => {
  ...
  const testRPCReliability = async (
    ...
    for (let i = 0; i < testCount; i++) {
      ...
      try {
        // Use axios with the eth_chainId method
        ...
        const expectedChainId =
          typeof chainData.chainId === 'string'
            ? parseInt(chainData.chainId, 10)
            : chainData.chainId;
              ...
              <Flex direction="column" style={{ flex: 1 }}>
                ...
                {status && (
                  <Text
                    size="1"
                    style={{
                      color: status.matchesChainId
                        ? 'var(--green-9)'
                        : 'var(--red-9)',
                    }}
                  >
                    {status.matchesChainId
                      ? 'Chain ID matches'
                      : 'Chain ID mismatch'}
                  </Text>
                )}
