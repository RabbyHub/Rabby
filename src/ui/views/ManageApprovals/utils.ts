...
const RiskNumMapMirrors = Object.entries(RiskNumMap).reduce(
  (acc, [k, v]) => {
    acc[v] = k;
    return acc;
  },
  { ...RiskNumMap } as typeof RiskNumMap & ObjectMirror<typeof RiskNumMap>
);
export function reEvaluateContractRisk(contract: ContractApprovalItem) {
  const $riskEval = contract.$contractRiskEvaluation;
  const trustValue = (() => {
    const isDanger = $riskEval.extra.clientSpendScore >= RiskNumMap.danger;
    const isWarning =
      !isDanger && $riskEval.extra.clientSpendScore >= RiskNumMap.warning;
  ...
  const revokeTrends = (() => {
    const isDanger = $riskEval.extra.clientApprovalScore >= RiskNumMap.danger;
    const isWarning =
      !isDanger && $riskEval.extra.clientApprovalScore >= RiskNumMap.warning;
  ...
  return {
    serverLevel: contract.risk_level as ApprovalRiskLevel,
    clientLevel,
    totalRiskScore,
...
export function getContractNFTType(
  ...
  if ('spender' in contractApprovalItem) {
    const maybeNFTSpender = contractApprovalItem.spender as SpenderInNFTApproval;
...
export function getFirstSpender(spenderHost: ApprovalItem['list'][number]) {
  if ('spender' in spenderHost) {
    return spenderHost.spender;
  }

  if ('spenders' in spenderHost) {
    return spenderHost.spenders[0];
  }
...
export function isSameRevokeItem(
  ...
  if ('nftTokenId' in src && 'nftTokenId' in target) {
    return (
      src.contractId === target.contractId &&
