...
export const CoinDropdown: React.FC<CoinDropdownProps> = ({
  ...
  const renderMarketRow = (item: MarketData) => {
    ...
    return (
      ...
      >
        ...
        <div className="flex flex-1">
          ...
          <div className={clsx('text-[13px] text-start flex-[1.5]')}>
            {isPositive ? '+' : '-'}$
          ...
          {/* 8hr Funding */}
          <div className={clsx('text-[13px] text-start flex-1')}>
          ...
          {/* Open Interest */}
          <div className="text-[13px] text-r-neutral-title-1 text-start flex-1">
            {formatUsdValueKMB(Number(item.openInterest) * Number(item.markPx))}
          </div>
        </div>
      </div>
    );
  ...
  const renderSortIcon = (field: SortField) => {
    ...
    return <span className="ml-[4px]">{sortOrder === 'asc' ? '↑' : '↓'}</span>;
  };

  const dropdownMenu = (
    <div className="bg-rb-neutral-card-1 rounded-[8px] shadow-lg border border-solid border-rb-neutral-line w-[800px] h-[480px] overflow-hidden flex flex-col px-16 pt-16">
      <Input
        ...
        className={clsx(
          'bg-rb-neutral-card1 border-rb-neutral-line focus:border-rb-brand-default placeholder-r-neutral-foot h-[40px] text-13 rounded-[6px]',
          searchInputFocused && 'border-rb-brand-default'
        )}
        allowClear
      ...
      <div className="flex items-center gap-[12px] px-[8px] py-[12px]">
        ...
        <div className="flex flex-1">
          ...
          <div
            className={clsx(
              'text-[13px] text-r-neutral-foot cursor-pointer hover:text-r-neutral-title-1 hover:font-medium transition-colors text-start flex-1',
              sortField === 'dayNtlVlm' && 'text-r-neutral-title-1 font-medium'
            )}
