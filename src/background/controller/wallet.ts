...
export class WalletController extends BaseController {
  ...
  // rejectApproval now accepts approvalId to prevent cross-request race conditions
  rejectApproval = (err?: string, stay = false, isInternal = false, approvalId?: string) => {
    return notificationService.rejectApproval(err, stay, isInternal, approvalId);
  };
  ...
  lockWallet = async () => {
    this.rejectAllApprovals(); // <-- Ensures pending approvals are safely cleared before lock boundary
